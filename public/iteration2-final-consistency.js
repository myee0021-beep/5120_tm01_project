...
function patchStaticCopy(){
 var p=page();
 if(p==='index'){
   walkText(document.body,function(s){return s
     .replace(/48,188/g,'47,033')
     .replace(/39,982/g,'39,766')
     .replace(/AT A GLANCE\s*·\s*E5/gi,'AT A GLANCE')
     .replace(/How review works\s*·\s*E5\s*,\s*E10/gi,'How review works')
     .replace(/Bagaimana semakan berfungsi\s*·\s*E5\s*,\s*E10/gi,'Bagaimana semakan berfungsi')
     .replace(/\s*·\s*E5\s*(?:to|hingga)\s*E7/gi,'');});
   document.querySelectorAll('h3').forEach(function(h){if(/Community/i.test(clean(h.textContent))){var card=h.closest('article,section,div');if(!card)return;var en=card.querySelector('[data-en]'),bm=card.querySelector('[data-bm]');if(en)en.textContent='Structured, anonymous reports from your district: what turned up, what worked, invasive species seen.';if(bm)bm.textContent='Laporan berstruktur dan tanpa nama daripada daerah anda: apa yang muncul, apa yang berkesan, spesies invasif dilihat.';}});
 }
 if(p==='ecosystem'||p==='ecosystem-map'){
   walkText(document.body,function(s){return s
     .replace(/By state, all seven species(?!, 2017 to 2026)/gi,'By state, all seven species, 2017 to 2026')
     .replace(/Mengikut negeri, semua tujuh spesies(?!, 2017 hingga 2026)/gi,'Mengikut negeri, semua tujuh spesies, 2017 hingga 2026');});
 }
 if(p==='species'){
   walkText(document.body,function(s){return s.replace(/See long-tailed macaque records on the map(?! \(2017 to 2026\))/gi,'See long-tailed macaque records on the map (2017 to 2026)');});
 }
 if(p==='ecosystem-redlist'||p==='species'){
   walkText(document.body,function(s){return s
     .replace(/5,807(?=\s+records\b)/g,'5,153')
     .replace(/330(?=\s+records\b)/g,'186')
     .replace(/3,710(?=\s+records\b)/g,'3,535')
     .replace(/431(?=\s+records\b)/g,'382')
     .replace(/209(?=\s+records\b)/g,'162')
     .replace(/18,021(?=\s+records\b)/g,'17,992')
     .replace(/19,680(?=\s+records\b)/g,'19,623');});
 }
}
...
function removeCombined(){
 ...
 document.querySelectorAll('#plan-result__speciesList div, #plan-print__sheetSpecies div, p, div').forEach(function(el){if(el.children.length>0&&el.id!=='plan-result__speciesList'&&el.id!=='plan-print__sheetSpecies')return;var t=clean(el.textContent);if(/^Combined level is not displayed until the D34 threshold row is available/i.test(t)||/^Tahap gabungan tidak dipaparkan sehingga baris ambang D34/i.test(t))el.remove();});
 walkText(document.body,function(s){return s.replace(/Combined level is not displayed until the D34 threshold row is available from one data source\.\s*/ig,'').replace(/Tahap gabungan tidak dipaparkan sehingga baris ambang D34[^.]*\.\s*/ig,'');});
}
function formatDates(){var p=page();if(!/plan-result|plan-print|emergency/.test(p))return;walkText(document.body,function(s){return s.replace(/\b(20\d{2}-\d{2}-\d{2})\b/g,function(x){return longDate(x);});});}
function applyD42Home(){if(page()!=='index')return;var select=document.getElementById('index__home_stateSelect');if(select){select.style.display='none';select.setAttribute('aria-hidden','true');select.tabIndex=-1;}var btn=document.getElementById('index__home_goBtn');if(!btn)return;if(btn.dataset.d42Bound!=='1'){btn.dataset.d42Bound='1';btn.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();location.hash='#plan';},true);}var box=btn.parentElement;if(box){box.style.paddingTop='0';box.style.gap='0';}}
...
function run(){patchStaticCopy();patchMacaque();removeCombined();formatDates();applyD42Home();addPrintMonths();ensureStyle();}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else setTimeout(run,0);
window.addEventListener('hashchange',function(){setTimeout(run,20);setTimeout(run,120);});document.addEventListener('roomforboth:pageshow',function(){setTimeout(run,20);setTimeout(run,120);});window.addEventListener('roomforboth:signals-ready',function(){setTimeout(run,0);setTimeout(run,100);});window.addEventListener('roomforboth:all-years-occurrence-data-ready',function(){setTimeout(run,0);});
new MutationObserver(function(){clearTimeout(window.__i2final);window.__i2final=setTimeout(run,120);}).observe(document.documentElement,{subtree:true,childList:true,characterData:true});
})();
...
function applyD42Home(){
 ...
 var select=document.getElementById('index__home_stateSelect');if(select){select.style.display='none';select.setAttribute('aria-hidden','true');select.tabIndex=-1;}
 var btn=document.getElementById('index__home_goBtn');if(!btn)return;
 if(btn.dataset.d42Bound!=='1'){btn.dataset.d42Bound='1';btn.addEventListener('click',function(e){e.preventDefault();e.stopImmediatePropagation();location.hash='#plan';},true);}
...
function addPrintMonths(){
 if(page()!=='plan-print'||!window.OCCURRENCES_ALL_YEARS||!Array.isArray(window.OCCURRENCES_ALL_YEARS.rows))return;
 var host=document.getElementById('plan-print__sheetSummary')||document.getElementById('plan-print__sheetSpecies');if(!host||document.getElementById('i2-print-months'))return;
 var st=stateKey(),codes=selectedCodes(),rows=window.OCCURRENCES_ALL_YEARS.rows,parts=[];
 codes.forEach(function(code){var m=new Array(12).fill(0),total=0;rows.forEach(function(r){if(norm(r[0])===st&&r[1]===code&&+r[3]>=1&&+r[3]<=12){m[+r[3]-1]+=+r[4]||0;total+=+r[4]||0;}});if(total>=30)parts.push(code.replace(/-/g,' ') + ': '+m.map(function(v,i){return MONTHS[i]+' '+v.toLocaleString();}).join(' · '));});
 if(!parts.length)return;var el=document.createElement('div');el.id='i2-print-months';el.className='mt-2 text-xs text-slate-600';el.innerHTML='<strong>Monthly profile</strong><br>'+parts.join('<br>');host.appendChild(el);
}
...
function run(){patchMacaque();removeCombined();formatDates();applyD42Home();addPrintMonths();ensureStyle();}
...
new MutationObserver(function(){clearTimeout(window.__i2final);window.__i2final=setTimeout(run,80);}).observe(document.documentElement,{subtree:true,childList:true,characterData:true});
})();
...
// Keep About > Spine datasets static: GBIF + MyBIS only.
...
// Prevention footer: show every distinct DB source, not only the first one.
api = api.replace(
...
// De-duplicate sources by actual URL/institution/person, not verification timestamp.
api = api.replace(
  `      var key = [\n        visibleText(row.source_url),\n        visibleText(row.source_institution),\n        visibleText(row.source_person),\n        visibleText(row.date_verified)\n      ].join('|');`,
  `      var key = [\n        visibleText(row.source_url),\n        visibleText(row.source_institution),\n        visibleText(row.source_person)\n      ].join('|');`
);

// Inline About renderer: this is deliberately independent of api-data.js
// render timing. render_about() can redraw its old static rows first; this
// renderer runs immediately afterwards and replaces them from the database.
...
(function () {
  ...
  function uniqueSources(rows) {
    ...
    return (rows || []).filter(function (row) {
      var url = clean(row && row.source_url);
      var inst = clean(row && row.source_institution);
      var person = clean(row && row.source_person);
  ...
  function sourceHtml(rows) {
    ...
    return '<div class="space-y-2">' + sources.map(function (row) {
      ...
      if (!url) return '<div class="text-slate-500">' + esc(label) + '</div>';
      return '<div><a href="' + esc(url) + '" target="_blank" rel="noopener" class="text-forest-600 hover:text-forest-800 font-semibold underline underline-offset-2">' + esc(label) + ' ↗</a></div>';
    }).join('') + '</div>';
  }
  ...
  function renderAboutDbSourcesV5() {
    ...
    return Promise.all(ABOUT_SPECIES.map(function (sp) {
      return Promise.all([
        ...
        return {
          name: sp.name,
          immediate: (result[0] && result[0].actions) || [],
          prevention: (result[1] && result[1].actions) || []
        };
      }).catch(function () {
        return { name: sp.name, immediate: [], prevention: [] };
      });
    })).then(function (rows) {
      body.innerHTML = rows.map(function (row) {
        return '<tr class="align-top">' +
          '<td class="py-4 pr-5 font-display font-bold text-forest-950">' + esc(row.name) + '</td>' +
          '<td class="py-4 pr-5 text-sm">' + sourceHtml(row.immediate) + '</td>' +
          '<td class="py-4 text-sm">' + sourceHtml(row.prevention) + '</td>' +
        '</tr>';
      }).join('');
  ...
  function install() {
    if (typeof window.render_about === 'function' && !window.render_about.__dbSourcesV5) {
      ...
      window.render_about = function () {
        ...
        setTimeout(renderAboutDbSourcesV5, 0);
        return result;
      };
    ...
    var current = '';
    try { current = (window.APP && window.APP.currentPage) || ''; } catch (e) {}
...
// Update the explanatory copy: source links are now direct external links.
html = html.replace(
  `Every "Source" note under an immediate-safety step or a prevention recommendation links here first,\n        rather than to an external page directly.`,
  `Database source links for every immediate-safety and prevention record are listed below.\n        Each link opens the original external source directly.`
);

fs.writeFileSync(workerPath, worker);
fs.writeFileSync(indexPath, html);
fs.writeFileSync(apiPath, api);
console.log('Installed inline About database source renderer v5.');

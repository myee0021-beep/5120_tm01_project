...
function stableRows(rows){
  return rows.map(function(row){
    return {
      prevention_id:row&&row.prevention_id!=null?String(row.prevention_id):'',
      action:clean(row&&row.action)
    };
...
function summaryCacheKey(rows,language){
  var stable=stableRows(rows).sort(function(a,b){
    var ai=Number(a.prevention_id),bi=Number(b.prevention_id);
    if(Number.isFinite(ai)&&Number.isFinite(bi)&&ai!==bi)return ai-bi;
    return (a.prevention_id+a.action).localeCompare(b.prevention_id+b.action);
  });
  var ids=stable.map(function(r){return r.prevention_id||'x';}).join('-');
  var fingerprint=hashText(JSON.stringify(stable));
  return new Request('https://plan-summary-cache.internal/v2/'+encodeURIComponent(language)+'/'+encodeURIComponent(ids)+'/'+fingerprint,{method:'GET'});
}
...
async function readCachedSummary(rows,language){
  try{
    ...
    if(payload&&payload.ok&&payload.summary&&validate(payload.summary,rows))return payload;
  }catch(e){console.warn('[plan-summary-cache] read failed',e?.message||e)}
  return null;
}

async function writeCachedSummary(rows,language,payload){
  try{
    var response=new Response(JSON.stringify(payload),{
      headers:{
        'content-type':'application/json; charset=utf-8',
        'cache-control':'public, max-age='+SUMMARY_CACHE_TTL
      }
    });
    await caches.default.put(summaryCacheKey(rows,language),response);
  }catch(e){console.warn('[plan-summary-cache] write failed',e?.message||e)}
}
...
async function handlePlanSummary(request,env,ctx){
  ...
  var cached=await readCachedSummary(rows,language);
  if(cached){
  ...
  for(var attempt=1;attempt<=2;attempt++){
    try{
      ...
      if(validate(text,rows)){
        ...
        if(ctx&&typeof ctx.waitUntil==='function')ctx.waitUntil(writeCachedSummary(rows,language,payload));
        else await writeCachedSummary(rows,language,payload);
...
export default{
  async fetch(request,env,ctx){
    ...
    if(request.method==='POST'&&url.pathname==='/api/i2/plan-summary')return handlePlanSummary(request,env,ctx);
    return iteration2Worker.fetch(request,env,ctx);
  }
};
